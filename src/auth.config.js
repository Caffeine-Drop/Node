...
const naverVerify = async (profile, accessToken, refreshToken) => {
  ...
  if (email == false) {
    throw new Error(`profile.email was not found: ${profile}`);  // 이메일이 없으면 에러 처리
  }
...
export const kakaoStrategy = new KakaoStrategy(
  ...
  (accessToken, refreshToken, profile, cb) => {
    return kakaoVerify(profile) //프로필을 이용해 유저를 검증하고, 반환된 유저 정보를 콜백함수로 전달
      .then((user) => cb(null, user)) //성공 시 유저 정보 전달
      .catch((err) => cb(err));  //실패 시 에러 전달
  }
);
...
const kakaoVerify = async (profile, accessToken, refreshToken) => {
  ...
  if (!email) {
    throw new Error(`profile.email was not found: ${profile}`); //카카오 프로필에서 이메일 추출
  }
  
  const user = await prisma.user.findUnique({ where: { email } });
  if (user !== null) {  //이미 유저가 존재하면 해당 유저 정보 반환환
  ...
  const createdUser = await prisma.user.create({
    data: {
      email,
      email_type: "kakao",
      nickname: profile.name || " ", //프로필 이름이 없으면 빈 문자열로 저장장
      profile_image_url: profile.profile_image,  //프로필 이미지 URL 저장장
      created_at: new Date(),
      updated_at: new Date(),
    },
  });
