...
class SearchController {
	async searchByKeyword(req, res, next) {
		...
		if (!lat || !lng || isNaN(lat) || isNaN(lng)) {
			return res
				.status(400)
				.json({ message: "위도와 경도는 필수이며 숫자여야 합니다." });
		}

		if (!radius || isNaN(radius)) {
			return res
				.status(400)
				.json({ message: "반경은 필수이며 숫자여야 합니다." });
		}
	...
	async getTopSearchTerms(req, res, next) {
		...
		if (isNaN(time)) {
			return res
				.status(400)
				.json({ message: "시간은 필수이며 YYYY-MM-DD HH:mm:ss 형식입니다." });
		}
	...
	async deleteAllSearchTerms(req, res, next) {
		...
		try {
			await service.deleteAllRecentSearches(user_id);
			return res.status(200).json({
